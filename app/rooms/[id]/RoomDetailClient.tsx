...
useEffect(() => {
  setRoom(initialRoom);
  setActiveIndex(0);
  privateFieldsFetchedKeyRef.current = "";
}, [initialRoom, roomId]);
...
const mediaSwipe = useDirectSwipeCarousel({
...
const fullscreenMediaSwipe = useDirectSwipeCarousel({
  count: mediaItems.length,
  index: activeIndex,
  onIndexChange: setActiveIndex,
  loop: false,
...
const moveFullscreenMedia = fullscreenMediaSwipe.move;
...
useEffect(() => {
  const onKeyDown = (event: KeyboardEvent) => {
    if (viewerOpen) {
      ...
      else if (event.key === "ArrowLeft") { event.preventDefault(); moveFullscreenMedia(-1); }
      else if (event.key === "ArrowRight") { event.preventDefault(); moveFullscreenMedia(1); }
      return;
    }
...
}, [handleCloseModal, moveFullscreenMedia, viewerOpen]);
...
return (
  ...
  >
    ...
    {!viewerOpen && activeItem?.kind === "video" && (
      <button
        type="button"
        ...
        className="absolute right-3 top-16 z-50 grid h-10 w-10 place-items-center rounded-full border border-white/20 bg-black/35 text-white shadow-[0_10px_30px_rgba(0,0,0,0.45)] backdrop-blur-[16px] transition hover:bg-black/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
        aria-label="Mở media toàn màn hình"
        title="Toàn màn hình"
      >
    ...
    {mediaItems.length > 0 ? (
      <>
        ...
        >
          ...
          {/* Ngày cập nhật: sát đáy ảnh; với video nằm trên native controls. */}
            {updatedText && (
              <div
                className={`
                  pointer-events-none
                  ...
                  ${activeItem?.kind === "video" ? "bottom-16" : "bottom-3"}
                `}
              >
        ...
        <div className="flex gap-2 overflow-x-auto pb-0">
          {mediaItems.slice(0, 20).map((it, idx) => (
            ...
            >
              {it.kind === "video" ? (
                <>
                  <video src={it.url} preload="none" playsInline muted className="pointer-events-none h-full w-full object-cover" />
...
{viewerOpen && mediaItems.length > 0 && (
  ...
  >
    <div
      ...
      {...fullscreenMediaSwipe.bind}
      onClick={(e) => e.stopPropagation()}
      style={{ touchAction: "none" }}
    >
      <div className={`flex h-full w-full will-change-transform ${fullscreenMediaSwipe.isAnimating ? "transition-transform duration-300 ease-[cubic-bezier(.22,1,.36,1)]" : ""}`} style={{ transform: fullscreenMediaSwipe.transform }} onTransitionEnd={fullscreenMediaSwipe.onTransitionEnd}>
        {fullscreenMediaSwipe.visibleIndexes.map((mediaIndex) => {
          const item = mediaItems[mediaIndex];
          const isCurrentSlide = mediaIndex === activeIndex;
      ...
      <button
        type="button"
        data-swipe-ignore="true"
        className="absolute right-4 top-16 z-[2147483647] grid h-11 w-11 place-items-center rounded-full border border-white/25 bg-white/10 text-white shadow-[0_14px_45px_rgba(0,0,0,0.55)] backdrop-blur-[24px] transition hover:bg-white/18 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
        onClick={() => setViewerOpen(false)}
