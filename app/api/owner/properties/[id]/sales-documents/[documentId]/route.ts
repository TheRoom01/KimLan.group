...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string; documentId: string }> }) {
  try {
    ...
    const { data: document, error: findError } = await supabase.from("property_documents").select("id,file_path").eq("id", salesDocumentId).eq("property_id", propertyId).maybeSingle();
    if (findError) return mapDatabaseError(findError);
    if (!document) return apiError("NOT_FOUND", "Không tìm thấy tài liệu", 404);
    const { error } = await supabase.from("property_documents").delete().eq("id", salesDocumentId).eq("property_id", propertyId);
