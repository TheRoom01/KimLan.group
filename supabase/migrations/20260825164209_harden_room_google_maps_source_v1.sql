...
create or replace function private.resolve_room_google_maps_source_v1(
  p_id uuid
)
returns jsonb
language plpgsql
...
revoke all on function private.resolve_room_google_maps_source_v1(uuid)
  from public, anon, authenticated, service_role;
grant execute on function private.resolve_room_google_maps_source_v1(uuid)
...
set search_path = ''
as $$
  select private.resolve_room_google_maps_source_v1(p_id);
$$;
...
comment on function private.resolve_room_google_maps_source_v1(uuid) is
  'Privileged implementation for the public security-invoker Maps source RPC.';
comment on function public.resolve_room_google_maps_source_v1(uuid) is
  'Returns the minimal authorized location source for one visible room.';
