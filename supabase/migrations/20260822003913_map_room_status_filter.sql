...
-- Add an optional status list without removing the previous overload yet.
-- This keeps the currently deployed web app compatible while the new UI is
-- rolled out. A null/empty list preserves the old default: available/upcoming.
...
comment on function public.search_rooms_in_map_v1(
  double precision, double precision, double precision, double precision,
  text, bigint, bigint, text[], text[], double precision, double precision,
  double precision, integer, text[]
) is 'Bounded Map Search with address visibility and optional multi-status filtering; null statuses retain the available/upcoming default.';
