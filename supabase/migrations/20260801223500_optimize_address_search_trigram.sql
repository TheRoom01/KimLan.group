...
create or replace function public.room_search_document(
  p_house_number text, p_address text, p_ward text, p_district text, p_room_code text
)
returns text
language sql
immutable
...
as $$
  select public.norm_room_search_text(
    concat_ws(' ', p_house_number, p_address, p_ward, p_district, p_room_code)
  );
$$;
...
alter table public.rooms
  ...
  generated always as (
    public.room_search_document(house_number, address, ward, district, room_code)
  ) stored;

create index if not exists rooms_search_text_trgm_idx
  on public.rooms using gin (search_text extensions.gin_trgm_ops);
...
begin
  ...
  with raw as (
    ...
    where (s is null or source_room.search_text operator(extensions.%) s)
