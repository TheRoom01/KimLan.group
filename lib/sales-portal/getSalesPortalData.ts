...
export const getSalesPortalData = cache(async function getSalesPortalData(token: string): Promise<SalesPortalData | null> {
  ...
  const { data: link } = await supabase
    ...
    .select("id, property_id, expires_at, revoked_at, last_accessed_at")
  ...
  const [propertyResult, documentsResult, roomsResult, notesResult] = await Promise.all([
    ...
    supabase.from("rooms").select("id, room_code, room_type, price, description, updated_at, chinh_sach, zalo_phone, status, lifecycle_status, room_media(id,type,url,is_cover,sort_order), rental_contracts(status,start_date,end_date,created_at)").eq("property_id", link.property_id).eq("lifecycle_status", "active").order("room_code"),
  ...
  const lastAccessedAt = link.last_accessed_at
    ? new Date(link.last_accessed_at).getTime()
    : 0;
  if (!Number.isFinite(lastAccessedAt) || now.getTime() - lastAccessedAt > 15 * 60 * 1000) {
  ...
  return {
    ...
    documents: (documentsResult.data ?? [])
      ...
      .map(({ sort_order, created_at, ...document }) => {
        void sort_order;
        void created_at;
        return { ...document, size_bytes: document.size_bytes == null ? null : Number(document.size_bytes) };
      }),
    rooms,
    summary,
  };
